...
var At = S((M) => {
  ...
  M.TEXT_ENCODER_THRESHOLD = Ot ? typeof process < "u" && ((Sr = process == null ? void 0 : process.env) === null || Sr === void 0 ? void 0 : Sr.TEXT_ENCODING) !== "force" ? 200 : 0 : us.UINT32_MAX;
  function xn(s, e, t) {
    e.set(He.encode(s), t);
  }
  function vn(s, e, t) {
  ...
  M.utf8EncodeTE = He != null && He.encodeInto ? vn : xn;
  var En = 4096;
  function Tn(s, e, t) {
    let r = e, n = r + t, o = [], i = "";
    for (; r < n; ) {
      ...
      if (!(c & 128))
        ...
        o.push((c & 31) << 12 | p << 6 | D);
      } else if ((c & 248) === 240) {
        let p = s[r++] & 63, D = s[r++] & 63, b = s[r++] & 63, j = (c & 7) << 18 | p << 12 | D << 6 | b;
  ...
  M.TEXT_DECODER_THRESHOLD = Ot ? typeof process < "u" && ((kr = process == null ? void 0 : process.env) === null || kr === void 0 ? void 0 : kr.TEXT_DECODER) !== "force" ? 200 : 0 : us.UINT32_MAX;
  function _n(s, e, t) {
...
var Wr = S((_) => {
  ...
  _.decodeTimestampToTimeSpec = fs;
  function Rs(s) {
    let e = fs(s);
    return new Date(e.sec * 1e3 + e.nsec / 1e6);
  }
  _.decodeTimestampExtension = Rs;
  _.timestampExtension = { type: _.EXT_TIMESTAMP, encode: ys, decode: Rs };
});
var Bt = S((Nt) => {
  "use strict";
...
var Ir = S((J) => {
  ...
  var Ar = class {
    constructor(e = On.ExtensionCodec.defaultCodec, t = void 0, r = J.DEFAULT_MAX_DEPTH, n = J.DEFAULT_INITIAL_BUFFER_SIZE, o = false, i = false, c = false, p = false) {
...
var Vt = S((Q) => {
  ...
  var Cr = ws(), Fn = Bt(), ce = Je(), jr = At(), Nr = Or(), qn = xs(), K = Ct(), Vn = (s) => {
    let e = typeof s;
  ...
  var vs = new Q.DataViewIndexOutOfBoundsError("Insufficient data"), Hn = new qn.CachedKeyDecoder(), Br = class {
    constructor(e = Fn.ExtensionCodec.defaultCodec, t = void 0, r = ce.UINT32_MAX, n = ce.UINT32_MAX, o = ce.UINT32_MAX, i = ce.UINT32_MAX, c = ce.UINT32_MAX, p = Hn) {
      this.extensionCodec = e, this.context = t, this.maxStrLength = r, this.maxBinLength = n, this.maxArrayLength = o, this.maxMapLength = i, this.maxExtLength = c, this.keyDecoder = p, this.totalPos = 0, this.pos = 0, this.view = Lr, this.bytes = Jn, this.headByte = Ge, this.stack = [];
    }
    reinitializeState() {
...
function dn(s) {
  ...
  if (s.every((r) => r && typeof r == "object" && !Pt(r) && !Ce(r))) {
    let r = s, n = r.every((i) => Object.keys(i).filter((c) => !Object.keys(r[0]).includes(c)).length === 0 && Object.keys(r[0]).filter((c) => !Object.keys(i).includes(c)).length === 0), o = r.every((i) => Object.values(i).every((c) => ls(c) || is(c)));
    if (n && o)
      return ee.fromD3(r);
  }
...
var ae = class extends x {
  ...
  set(t, r) {
    ...
    try {
      ...
      let i = new ae(r);
      w(i, n);
      let c = new U("[[<-"), p = l._Rf_lang4(c.ptr, this.ptr, o.ptr, i.ptr);
...
var ee = class extends qe {
  ...
  static fromObject(e) {
    ...
    try {
      let o = !!t && t.length > 0 && t.every((c) => c), i = r.length > 0 && r.every((c) => Array.isArray(c) || ArrayBuffer.isView(c) || c instanceof ArrayBuffer);
      if (o && i) {
        let c = r, p = c.every((b) => b.length === c[0].length), D = c.every((b) => ls(b[0]) || is(b[0]));
        if (p && D) {
          let b = new qe({ type: "list", names: t, values: c.map((tn) => os(tn)) });
          w(b, n);
...
var te = class extends y {
  constructor(e, t, r) {
    ...
    let n = { n: 0 };
    try {
      let { names: o, values: i } = me(e), c = l._Rf_allocVector(N[t], i.length);
      w(c, n), i.forEach(r(c)), y.wrap(c).setNames(o), super(new x(c));
    } finally {
      P(n.n);
    }
  }
